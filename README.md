...
The first run automatically creates `turo.db` (a SQLite database file) in this folder — that's where all your data lives. Back it up by copying that one file.

The very first time you open the app, you'll be asked to create an account (username + password) — this becomes the first partner/user. Once logged in, add accounts for other partners from the **Users** page. Everyone with an account has equal, full access to add/edit/delete anything.

### Accessing from another device

- Make sure the other device is on the same WiFi network as the computer running the app.
- Use the "Network" URL printed on startup (it changes if your computer's local IP changes — e.g. after reconnecting to WiFi).
- If it doesn't load, your computer's firewall may be blocking incoming connections on port 5050 — you may need to allow Python/this app through your firewall (macOS: System Settings → Network → Firewall; Windows: Windows Defender Firewall → Allow an app).
- Each person logs in with their own account; sessions stay logged in for 30 days per browser. This is basic access control for a trusted home network, not hardened security — still don't port-forward this app to the public internet.
- To restrict it back to just this computer, run with `HOST=127.0.0.1 python3 app.py`.

## Importing from Turo

Click **Import CSV** in the top bar and upload the trip earnings export from Turo's Host Tools → Earnings page.

- Each row becomes one income transaction (category "Trip payout") using Turo's "Total earnings" column, which is already net of discounts, Turo's fees, and add-ons — so no double counting.
- The first time you import, you'll be asked to match each vehicle in the file to a car — either link it to an existing car or let it create a new one automatically (named after your Turo listing title). After that, the same vehicle auto-links on future imports.
- Every trip is shown in a preview table before anything is saved, with a checkbox per row. Completed and in-progress trips are checked by default; upcoming "Booked" trips and no-fee cancellations are unchecked by default since nothing's been earned yet — check them manually if you want to include them.
...
- This runs Flask's built-in development server, which is fine for personal/local use on your home network. Login is basic (username/password, no lockouts or 2FA) — don't port-forward it or otherwise expose it to the public internet as-is.
...
- To reset all data, stop the app and delete `turo.db` — it will be recreated empty on next run.
...
- **Tire tracking**: same pattern as oil changes — set a per-car or fleet-wide default interval (Settings), log tire changes, get a due badge.
...
- **Ownership %**: per-partner ownership stake per car (Edit Car page), for future profit-split/K-1 prep — not yet used in any calculation.
